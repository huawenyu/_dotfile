...
# Script to report disk usage in the home directory and identify large files/directories

# Define the home directory
HOME_DIR="$HOME"

# Define the output file for the report
REPORT_FILE="$HOME_DIR/disk_usage_report.txt"

# Function to generate the disk usage report
generate_report() {
    echo "Generating disk usage report for $HOME_DIR..."
    echo "Report will be saved to $REPORT_FILE"
    echo ""

    # Get total disk usage of the home directory
    echo "=== Total Disk Usage ===" > "$REPORT_FILE"
    du -sh "$HOME_DIR" >> "$REPORT_FILE"
    echo "" >> "$REPORT_FILE"

    # Get disk usage by directory (sorted by size)
    echo "=== Disk Usage by Directory (Top 10) ===" >> "$REPORT_FILE"
    du -h --max-depth=1 "$HOME_DIR" 2>/dev/null | sort -hr | head -n 11 >> "$REPORT_FILE"
    echo "" >> "$REPORT_FILE"

    # Find large files (e.g., > 100MB)
    echo "=== Large Files (> 100MB) ===" >> "$REPORT_FILE"
    find "$HOME_DIR" -type f -size +100M -exec du -h {} + 2>/dev/null | sort -hr >> "$REPORT_FILE"
    echo "" >> "$REPORT_FILE"

    # Check for hidden files and directories
    echo "=== Large Hidden Files/Directories ===" >> "$REPORT_FILE"
    find "$HOME_DIR" -name ".*" -type f -size +50M -exec du -h {} + 2>/dev/null | sort -hr >> "$REPORT_FILE"
    echo "" >> "$REPORT_FILE"

    echo "Report generation complete. Check $REPORT_FILE for details."
}
