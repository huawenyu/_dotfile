...
# Get local user define info
local_info="$( [ -f /etc/info ] && cat /etc/info || echo 'Customer info /etc/info' )"
...
local_ip="$(hostname -I)"
#local_ip="$(ip addr show | grep 'inet ' | cut -d' ' -f6| cut -d/ -f1 )"
...
${C1} + ${C3}Hostname       ${C1}=  ${C4}$(hostname) ${C0}($(hostname --fqdn)) ${C6}${local_info}
${C1} + ${C3}IPv4 Address   ${C1}=  ${C4}$remote_ip  ${C6}Address: ${local_ip}
